...
class ChooseCard extends Component {
    ...
    handleSubmitBuy = async () => {
        ...
        if (this.state.listCard.length > 0) {
            //do something
            try {
                await handleOrder({ userId: userInfo._id }, this.props.accessToken)
            } catch (e) {
                await this.props.generateNewToken(this.props.refreshToken, userInfo._id)
                await handleOrder({ userId: userInfo._id }, this.props.accessToken)
            }
...
const mapStateToProps = state => {
    return {
        ...
        refreshToken: state.app.accessToken
    };
};

const mapDispatchToProps = dispatch => {
    return {
        generateNewToken: (refreshToken, userId) => dispatch(handleGenerateNewToken(refreshToken, userId))
    };
};
